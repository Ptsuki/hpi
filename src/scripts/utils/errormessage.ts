...
export default async function errorMessage(error: unknown) {
	...
	function displayMessage(dataStr: string) {
		...
		title.textContent = `Oops  failed 😖`
		subtitle.textContent = `Copy the error and your settings below and contact us !`
		explain.textContent =
			'Sharing your settings with us helps a lot in debugging. You can also reset , or close this window for now if you think it is a false alert.'
		...
		resetButton.textContent = 'Reset ohi'
