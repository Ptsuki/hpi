...
export async function settingsInit() {
	...
	initInput('i_qttype', data.quotes?.type || 'c=f')
	...
	// No bookmarks import on safari || welcome🥲
	if (BROWSER === 'safari' || PLATFORM === 'welcome🥲') {
	...
	if (data?.unsplash?.collection) {
		...
		paramId('i_collection')?.setAttribute('placeholder', coll ? coll : 'S24wUffrjDY')
	}
...
/* 	paramId('i_usdate').addEventListener('change', function (this: HTMLInputElement) {
	...
	const isOnline = PLATFORM === 'welcome🥲'
